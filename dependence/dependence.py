...
def do_work(job_num):
    ...
    data_list = read_parquet_files(os.path.join('..', 'original_data_parquet'))
            ...
            for model_param in ['default', 'overfit1', 'overfit2', 'overfit3']:
                for rep in reps:
                    if this_job < int(job_num):
                        this_job += 1
                        continue
                    res_path = os.path.join('dependence_results', f"results.{dataset['file_name']}.{col}.{model_param}.{rep}.json")
                    ...
                    print(f"Do dependence test for column {col}")
...
def read_json_files_to_dataframe(directory, force=False):
    res_path = os.path.join('dependence_results', 'dependence_results.parquet')
...
def do_gather():
    _ = read_json_files_to_dataframe('dependence_results', force=True)
...
def do_plot():
    df = read_json_files_to_dataframe('dependence_results', force=False)
    print(df.columns)
