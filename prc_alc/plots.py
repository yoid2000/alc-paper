...
def plot_prec_recall_for_diff_rmin(out_name):
    ...
    for rmin in rmins:
        ...
        plt.scatter(recall_values, prec_values, label=f'Cmin = {rmin}', s=1)
    plt.xscale('log')
    plt.grid(True)
    plt.ylim(0.4, 1.05)
    plt.xlabel('Coverage', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    plt.text(0.05, 0.98, f'Fa = {prc_val}, alpha = {alpha}', ha='left', va='top', fontsize=9, transform=plt.gca().transAxes)
...
def plot_prec_recall_for_diff_alpha(out_name):
    ...
    plt.xlabel('Coverage', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    plt.text(0.05, 0.98, f'Fa = {prc_val}, Cmin = 0.0001', ha='left', va='top', fontsize=9, transform=plt.gca().transAxes)
    plt.legend(scatterpoints=1, markerscale=7, handletextpad=0.5, labelspacing=0.5, fontsize='small', loc='upper right')
    plt.tight_layout()
    savefigs(plt, out_name)
...
def plot_prec_recall_for_equal_prc(out_name):
    ...
    for prc_val in prc_vals:
        ...
        prec_recall_pairs = [(prec, recall) for prec, recall in prec_recall_pairs if prec <= 1.0]
        prec_values, recall_values = zip(*prec_recall_pairs)
        plt.scatter(recall_values, prec_values, label=f'Fa = {prc_val}', s=1)
    plt.xscale('log')
    plt.grid(True)
    plt.ylim(-0.1, 1.1)
    plt.xlabel('Coverage', fontsize=12)
...
def run_prc_checks(alc, p_base, r_base, prr_base):
    if r_base <= 0.0001:
        return
    p_base_test = round(alc.prec_from_prc_recall(prr_base, r_base),2)
    if round(p_base,2) != p_base_test:
        print(f'Error: prec_from_prc_recall({prr_base}, {r_base})')
        print(f'Expected: {round(p_base,2)}, got: {p_base_test}')
        quit()
    r_base_test = round(alc.recall_from_prc_prec(prr_base, p_base),2)
    if round(r_base,2) != r_base_test:
        print(f'Error: recall_from_prc_prec({prr_base}, {p_base})')
        print(f'Expected: {round(r_base,2)}, got: {r_base_test}')
        quit()
...
def do_alc_test(alc, p_base, r_base, increase, r_attack):
    ...
    print(f'ALC: {round(alc.alc(p_base=p_base, r_base=r_base, p_attack=p_attack, r_attack=r_attack),2)}')
...
def make_alc_plots(recall_adjust_strength=3.0, pairs='v3'):
    ...
    savefigs(plt, f'alc_plot_{recall_adjust_strength}_{pairs}')
...
def plot_basic_alc(alc):
    ...
    ax.set_xlabel('Fa Base', fontsize=16)
    ax.set_ylabel('Fa Attack', fontsize=18)
...
def plot_recall_alc(alc):
    ...
    for recall in recall_values:
        ...
        ax.plot(filtered_p_base_values, filtered_p_atk_values, label=f'Coverage = {recall}', linewidth=3)

    # Set the labels with larger font size
    ax.set_xlabel('Accuracy Base', fontsize=16)
    ax.set_ylabel('Accuracy Attack', fontsize=18)
...
def plot_alc_scatter(alc):
    ...
    labels = [f'coverage_attack/coverage_base = {1/c_ratio}' for c_ratio in c_ratios]
    ...
    ax1.set_title(f'Baseline and Attack Accuracy = {p_base}', fontsize=12)
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.set_title(f'Baseline and Attack Accuracy = {p_base}', fontsize=12)
    ax2.legend()
    ax2.grid(True)
